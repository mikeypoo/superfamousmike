...
class GameplayScene extends Phaser.Scene {
    ...
    randomWallImg() {
        if (Math.random() < 0.15) {
            ...
            return ['cardboardan', 'the_weather', 'pad_thai'][Math.floor((Math.random()*3))]
        }
    }
    ...
    drawSmallMembers(context) {
        for (let i = 0; i < 8; i++) {
            const smallMember = context.add.image(95 + (i * 50), 160, `small_member_${i}`)
    ...
    create() {
        ...
        this.casualtyText = this.add.text(70, 16, 'x 5', { fontSize: '32px', fill: '#000' })
        this.memberText = this.add.text(100, 80, 'members to pick up', { fontSize: '32px', fill: '#000' })
        this.scoreText = this.add.text(496, 16, '0', { fontSize: '32px', fill: '#000', align: 'right'})
        ...
        this.wall = this.physics.add.sprite(600, this.randomWallPosition(), this.randomWallImg())
        this.physics.add.overlap(this.vandy, this.wall, this.casualty, null, this)
        this.physics.add.overlap(this.vandy, this.collisionGoalRect, this.detectGoalCollision, null, this)
        ...
        const memberInterval = setInterval(() => {
            ...
            if (!this.showingActiveMember) {
                ...
                if (this.withinRange(randomXPosition, vandyBounds.x, vandyBounds.x + vandyBounds.width) && this.withinRange(randomYPosition, vandyBounds.y, vandyBounds.y + vandyBounds.height)) {
                    return null
                } else {
                    this.activeMember = this.physics.add.sprite(randomXPosition, randomYPosition, this.randomMemberImg())
                    this.physics.add.overlap(this.vandy, this.activeMember, this.gatherMember, null, this)
    ...
    randomMemberImg() {
        ...
        const remainingIndices = allIndices.filter(num => !this.gatheredMembers.includes(num))
        const randyNumberman = Math.floor(Math.random() * 100)

        if (randyNumberman >= 70 && remainingIndices.length > 0) {
            return 'member_' + remainingIndices[Math.floor((Math.random()*remainingIndices.length))].toString()
        } else {
            return 'member_' + allIndices[Math.floor((Math.random()*allIndices.length))].toString()
        }
...
class CasualtyScene extends Phaser.Scene {
    ...
    randomCasualtyScene() {
        return ['casualty_0', 'casualty_1', 'casualty_2'][Math.floor((Math.random()*3))]
    }
